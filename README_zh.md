...
- **RS-485 支持**：通过 RTS 实现内核级 RS-485 半双工方向控制
- **串口诊断**：内置 `serial-dump` 命令，用于调试 RS-485/串口连接问题
...
| `--slave-id` | `-s` | Modbus 从站/设备地址（1-247） | `1` |
...
| 标志 | 简写 | 描述 | 默认值 |
|------|------|------|--------|
| `--port` | `-p` | 串口设备路径 | `/dev/ttyAMA3` |
| `--baud` | `-b` | 波特率 | `9600` |
| `--data-bits` | `-d` | 数据位（7 或 8） | `8` |
| `--stop-bits` | | 停止位（1 或 2） | `1` |
| `--parity` | | 校验：none、even、odd | `none` |
| `--rts-low` | | 强制 RTS 拉低（启用 RS-485 接收模式） | `true` |
| `--dtr-low` | | 强制 DTR 拉低 | `false` |
| `--rs485` | | 启用内核 RS-485 模式（自动 RTS 控制） | `false` |
...
# Modbus 从站/设备地址（1-247，默认：1）
# 对于 RTU 模式，这是串行总线上的站地址
# 对于 TCP 模式，这是 Unit ID
slave_id: 1
...
serial:
  ...
  # RS-485 半双工方向控制（可选，用于 RTU 模式）
  # 启用后，内核自动控制 RTS 进行收发切换。
  # rs485:
  #   enabled: true
  #   rts_high_during_send: true    # 发送时 RTS 拉高
  #   rts_high_after_send: false    # 发送后 RTS 拉低（启用接收）
  #   rx_during_tx: false           # 全双工 RS-485（罕见）
  #   delay_rts_before_send_ms: 0   # 发送前延迟（毫秒）
  #   delay_rts_after_send_ms: 1    # 发送后切换到接收前的延迟（毫秒）
...
`serial-dump` 命令是一个诊断工具，用于调试 RS-485/串口连接问题。它以完整的 RTS/DTR 控制方式打开串口，并以十六进制格式显示接收到的数据。

#### 基本用法

```bash
# 转储串口数据，RTS 默认拉低（启用 RS-485 接收模式）
sudo ./modbus-sim serial-dump -p /dev/ttyAMA3 -b 9600

# 禁用 RTS 控制（适用于 RS-232 端口）
sudo ./modbus-sim serial-dump -p /dev/ttyUSB0 --rts-low=false

# 启用内核 RS-485 模式，自动方向控制
sudo ./modbus-sim serial-dump -p /dev/ttyAMA3 --rs485

# 查看 modem 线路状态用于诊断
sudo ./modbus-sim serial-dump -p /dev/ttyAMA3
# 输出包括：RTS=high/low CTS=high/low DTR=high/low DSR=high/low CD=high/low RI=high/low
```

#### RS-485 常见问题

如果 RS-485 设备无法接收数据，但接真实设备可以工作，可能是收发器卡在发送模式，因为 **RTS 默认为高电平**。`serial-dump` 命令默认 `--rts-low=true`，强制 RTS 拉低以启用接收模式。

对于生产环境的 RTU 服务器，建议在 YAML 配置中启用 RS-485 配置，让内核自动处理 RTS 切换。
...
└── internal/
    ...
    │
    ├── serial/                 # 原始串口操作（Linux）
    │   └── port.go             # RTS/DTR 控制、RS-485 内核模式
    │
    └── server/                 # 网络服务器
        ├── server.go           # TCP/RTU 服务器实现
        └── handlers.go         # 请求/响应日志处理器
```
...
#### 5. RS-485 无法接收数据（RTU 模式）

**问题：** 串口打开成功但收不到数据（超时错误），而同一端口接真实设备可以正常工作。

**根本原因：** RS-485 收发器使用 RTS 信号控制收发方向。当 RTS 为高电平（默认值）时，收发器处于发送模式，无法接收数据。

**解决方案：**

```bash
# 1. 诊断：使用 serial-dump 检查 modem 线路状态
sudo ./modbus-sim serial-dump -p /dev/ttyAMA3 -b 9600
# 在 modem 线路状态输出中查看 "RTS=high"

# 2. 快速修复：serial-dump 默认 --rts-low=true
sudo ./modbus-sim serial-dump -p /dev/ttyAMA3 -b 9600
# 应显示 "RTS set LOW (RS-485 receive mode enabled)"

# 3. 生产环境：在配置中启用 RS-485 内核模式
```
...
serial:
  ...
  rs485:
    enabled: true
    rts_high_during_send: true   # 发送时 RTS 拉高
    rts_high_after_send: false    # 发送后 RTS 拉低
    rx_during_tx: false
    delay_rts_before_send_ms: 0
    delay_rts_after_send_ms: 1   # 切换到接收前的延迟
```
